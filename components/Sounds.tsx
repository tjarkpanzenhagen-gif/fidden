...
const SC_URL =
  "https://w.soundcloud.com/player/?url=https%3A//soundcloud.com/flying-angel-88722887" +
  ...
  const onScriptReady = useCallback(() => {
    ...
    w.bind(window.SC.Widget.Events.READY, () => {
      w.getSounds((sounds: SCTrack[]) => {
        if (!sounds || sounds.length === 0) { setStatus("error"); return; }
        setTracks(sounds);
        setStatus("ready");
      });
    });
    ...
    w.bind(window.SC.Widget.Events.PLAY_PROGRESS, (e: { currentPosition: number; loadedProgress: number; relativePosition: number }) => {
        ...
        <RevealSection delay={120}>
          ...
          <div className={`sc-now-playing${nowPlaying ? "" : " hidden"}`}>
            ...
            <div className="sc-np-controls">
              ...
              <a
                href="https://soundcloud.com/flying-angel-88722887"
                target="_blank"
          ...
          {/* Track list */}
          {status === "loading" && (
            <div className="sc-loading">Tracks werden geladen…</div>
          )}
          {status === "error" && (
            <div className="sc-loading sc-error">
              Tracks konnten nicht geladen werden.{" "}
              <a href="https://soundcloud.com/flying-angel-88722887" target="_blank" rel="noopener noreferrer" style={{ color: "inherit", textDecoration: "underline" }}>
                Direkt auf SoundCloud öffnen →
              </a>
            </div>
          )}
          {status === "ready" && (
          ...
          <div className="sc-sc-link">
            <a href="https://soundcloud.com/flying-angel-88722887" target="_blank" rel="noopener noreferrer">
